...
pub fn fast_batched_dot_product_other<const K: usize, T: Copy>(
    params: &Params,
    c: &mut [u64],
    a: &[u64],
    a_elems: usize,
        ...
        for k_outer in 0..num_chunks {
            for j_outer in 0..j_num_chunks {
                for j_inner in 0..j_chunk_size {
                    ...
                    let mut total_sum_lo = [_mm256_setzero_si256(); K];
                    let mut total_sum_hi = [_mm256_setzero_si256(); K];
                    let mut tmp = [_mm256_setzero_si256(); K];
                    ...
                    for k_inner in 0..chunk_size {
                        ...
                        let a_idx = k;
                        let b_idx = j * b_rows + k;
                        let b_val_simd = b_ptr.add(b_idx).to_m256();

                        for batch in 0..K {
                            tmp[batch] =
                                _mm256_load_si256(a_slcs[batch].as_ptr().add(a_idx) as *const _);
                        }

                        for batch in 0..K {
                            let a_val_lo = tmp[batch];
                            let a_val_hi = _mm256_srli_epi64(tmp[batch], 32);

                            total_sum_lo[batch] = _mm256_add_epi64(
                                total_sum_lo[batch],
                                _mm256_mul_epu32(a_val_lo, b_val_simd),
                            );
                            total_sum_hi[batch] = _mm256_add_epi64(
                                total_sum_hi[batch],
                                _mm256_mul_epu32(a_val_hi, b_val_simd),
                            );
                        }
                    }
...
mod test {
    ...
    #[test]
    fn test_fast_batched_dot_product() {
        ...
        for _ in 0..trials {
            ...
            fast_batched_dot_product::<A_ROWS, _>(
                &params,
                c.as_mut_slice(),
                a.as_slice(),
                a_cols,
